...
## 0. Status do documento — leia antes de tudo

Este documento foi escrito na Rodada 2, antes da Rodada 2.5 (naming
canônico). **Várias decisões de naming aqui foram revisadas na Rodada
2.5** e o documento canônico para essas decisões é
`cvmdata_rodada2-5_naming_unificado.md` (a ser inserido nos arquivos
do projeto ao fechamento da Rodada 2.5). Quando houver conflito entre
este documento e o de naming, **prevalece o de naming**.

As atualizações cirúrgicas pontuais foram inseridas nas seções
afetadas com marcação **[ATUALIZADO ROD 2.5]**. O corpo histórico do
texto foi preservado para registro do raciocínio original; quando
relevante, o texto original aparece riscado ou anotado.

### 0.1 Régua de idioma vigente (substitui o texto anterior)

A Rodada 2 deste documento havia estipulado "todo código, schema,
identificador, mensagem do pacote e documentação CRAN em inglês". A
Rodada 2.5 reformulou essa régua para:

**Princípio reitor**: se vem da CVM, fica como na CVM (apenas com
normalização tipográfica para snake_case minúsculo). Se não vem da
CVM, é decisão do pacote.

**Em inglês**: funções, argumentos, atributos de tibble, classes de
condição, options/env vars, nomes de arquivo e pasta, comentários em
código, README, NEWS, DESCRIPTION, roxygen docs, mensagens de erro,
workflows CI.

**Em português** (snake_case minúsculo): nomes de tabela, nomes de
coluna do tibble retornado por `cvm_fetch()`, colunas-meta retornadas
por funções de descoberta (`cvm_datasets`, `cvm_tables`,
`cvm_dictionary`), valores categóricos de células, texto livre
descritivo.

### 0.2 Substituições de naming aplicadas

A tabela abaixo lista, em ordem alfabética, todos os identificadores
que apareciam no texto original e foram revisados na Rodada 2.5. As
seções listadas em "Onde aparecia" continuam no documento com o texto
original; o leitor deve mentalmente aplicar a substituição.

| Identificador na Rodada 2 | Identificador travado na Rodada 2.5 | Onde aparecia |
|---|---|---|
| `fetch_cvm()` | `cvm_fetch()` | §3.1 e seguintes |
| `fetch_cad()` | `cad_fetch()` | §3.2.1 |
| `fetch_itr()` | `itr_fetch()` | §3.2.2 |
| `fetch_dfp()` | `dfp_fetch()` | §3.2.3 |
| `fetch_fre()` | `fre_fetch()` | §3.2.4 |
| `fetch_fii()` | `fii_fetch()` | §3.3 (ilustrativo) |
| `fetch_fidc()` | `fidc_fetch()` | §3.3 (ilustrativo) |
| `cnpj` (coluna) | `cnpj_cia` | §3.5.3, §4.x |
| `cnpj_clean` (coluna) | removida; valor canônico de `cnpj_cia` é com pontuação | §3.5.3 |
| `cvm_code` (coluna) | `cd_cvm` | §3.5.3, §4.x |
| `company_name` (coluna) | `denom_cia` | §3.5.3 |
| `reference_date` (coluna) | `dt_refer` | §3.5.3, §4.2 |
| `period_start_date` (coluna) | `dt_ini_exerc` | §4.2.3 |
| `period_end_date` (coluna) | `dt_fim_exerc` | §4.2.3 |
| `version` (coluna) | `versao` | §3.5.3 |
| `bpa_con`, `dre_ind`, etc. (nomes de tabela) | **mantidos** (siglas oficiais CVM em PT) | §4.2.1 |
| `composicao_capital` (nome de tabela) | **mantido** (não traduzido para `capital_composition`) | §4.2.4 |
| `parecer` (nome de tabela) | **mantido** (não traduzido para `audit_opinion`) | §4.2.5 |
| `Orgao_Administracao` (coluna FRE) | `orgao_administracao` | §4.4.3 |
| Valores de `governing_body` em EN | mantidos em PT como vêm da CVM (`"Conselho de Administração"`, etc.) | §4.4.3 |
| Hierarquia de classes de condição | confirmada: `cvmdata_error` + 4 sub-classes (input, http, parse, internal) | §3.7.1 |
| Argumento `validate` (TRUE/FALSE booleano) | `validate = "strict"/"warn"/"skip"` (três níveis) | §3.1.1 |
| Argumento `on_error` (TRUE/FALSE booleano) | `on_error = "abort"/"warn"/"silent"` (três níveis) | §3.1.1 |
| Atributos `source`, `fetched_at` etc. | confirmados em **inglês** | §3.5.2 |
| Option de verbosidade | travada: `cvmdata.verbosity` com `"quiet"/"normal"/"debug"` | §3.8 |

### 0.3 Adições da Rodada 2.5 não previstas na Rodada 2

- **Função utilitária `cnpj_clean(x)`** exportada na v0.1 — gera versão
  do CNPJ sem pontuação para joins externos.
- **`cvm_dictionary(dataset, table)`** com **ambos argumentos
  obrigatórios** e retorno com 7 colunas espelhando o dicionário
  oficial CVM (`campo`, `descricao`, `tipo_dados`, `dominio`,
  `tamanho`, `precisao`, `scale`).
- **`cvm_datasets()`** com retorno de 5 colunas: `conjunto_dados`,
  `grupo`, `descricao`, `n_tabelas`, `primeiro_ano`. A coluna `grupo`
  herda a taxonomia oficial CVM (~54 valores possíveis).
- **`cvm_tables(dataset)`** com retorno de 3 colunas: `tabela`,
  `descricao`, `n_colunas`.
- **Família de cache em 4 funções**: `cvm_cache_path()`,
  `cvm_cache_set_path()`, `cvm_cache_clear()`, `cvm_cache_info()`.
- **Família de source em 2 funções**: `cvm_source_get()`,
  `cvm_source_set()`. Domínio aceito: `"auto"`/`"portal"`/`"mirror"`.
- **YAMLs de schema** simplificados: cada tabela tem YAML mínimo em
  `inst/extdata/schemas/<dataset>/<table>.yaml` com mapeamento de
  arquivo CVM → tabela tidy e seção opcional `transformations:` para
  documentar remoções/transformações (ex.: `vl_conta` × `escala_moeda`
  com remoção de `escala_moeda`). Dicionário CVM vive em snapshot
  separado (não duplicado por tabela).

### 0.4 Pendências empíricas para Rodada 3

A Rodada 2.5 registrou as seguintes pendências cuja resolução exige
verificação direta nos arquivos da CVM (impraticável sem ferramentas
de execução na sessão de planejamento):

- Nome do arquivo de cabeçalho do ITR/DFP (originalmente chamado de
  `header` no plano; nome real do arquivo CSV-CVM a confirmar).
- Verificação se `cd_cvm` admite zeros à esquerda — afeta tipo (string
  vs integer; travamos provisoriamente **character** por princípio de
  segurança).
- Verificação se `versao` admite formatos não-numéricos — idem.
- Conteúdo exato do dicionário CVM para cada tabela (campos `dominio`,
  `tamanho`, `precisao`, `scale`).
- Codelist completa de cada coluna `dominio`.
- Eventuais colunas constantes em algum dataset (perguntar a Sidney
  caso a caso).
- Nome do arquivo `cvm_file_pattern` para cada tabela.

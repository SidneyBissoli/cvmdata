...
year-partition filter pushdown; `"cvm"` reads the open-data portal
over HTTP and remains fully supported. The current backend is read by
`cvm_source_get()` and changed at session scope by `cvm_source_set()`:

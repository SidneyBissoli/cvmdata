---
title: "issuer_fetch(): the generic API"
---
...
`issuer_fetch(dataset, table, ...)` is the single entry point for
every CVM issuer dataset and every table covered by the package.

This article walks through each argument, the issuer-detection
logic, the `year` semantics, the `report_type` rule, the `validate`
and `on_error` modes, and the cache/source plumbing.
...
such a table, `issuer_fetch()` reads the dataset's `submissao` for the
...
tidyverse conventions). `NULL` (default) fetches the **latest
...
of year N − 2. `issuer_fetch()` does not synthesize the antepenultimate;
ask for `year = (N - 2):N` if you need it.
...
calls, non-yearly tables (CAD) and the implicit `year = NULL`

...
claude.ai (ou equivalente). É escrito como instruções diretas ao
assistant.
...
(setor de pesquisa do Senado Federal), mantenedor do pacote `educabR` no
CRAN. Tem conhecimento intermediário-avançado em R (tidyverse, ggplot2,
Quarto, Shiny, renv, GitHub) e em metodologia de pesquisa. Pacote
estatístico principal: R. Trabalha solo, com assistência de LLM, em
tempo parcial.

## Idioma — régua canônica

A régua de idioma é **diferenciada por tipo de elemento** e foi
estabelecida na Rodada 2.5 após análise dos padrões CRAN, rOpenSci e dos
precedentes de pacotes do mesmo domínio (`rb3`, `microdatasus`,
...
- Nomes de **option/env var** (`cvmdata.verbosity`,
  `CVMDATA_VERBOSITY`).
...
- **Nomes de colunas** nos tibbles retornados por
  [`cvm_fetch()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_fetch.md)
  e aliases. Preservados **exatamente como vêm dos arquivos da CVM**,
...
- **Valores de células** nas colunas categóricas
  (`"Conselho de Administração"`, `"Diretoria Estatutária"`, `"ÚLTIMO"`,
  `"PENÚLTIMO"`, `"REAL"`, `"MIL"`, `"UNIDADE"`, etc.). Preservados como
  vêm da CVM.
...
permanece em inglês — palavra técnica que entrou no vocabulário PT sem
tradução natural na comunidade R. A coluna paralela no retorno de
[`cvm_datasets()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_datasets.md)
chama-se `conjunto_dados` (em PT, fiel à nomenclatura oficial da CVM no
portal `dados.cvm.gov.br`). Este mismatch deliberado é documentado no
help.

### Se o usuário pedir comentários de código em português

Atender, mas avisar do trade-off com a política CRAN de discoverability
internacional.
...
- Funções: fully-qualified namespaces sempre que apareçam fora do
  ...
  Dentro do pacote, declarar `@importFrom` no roxygen2 e usar nome bare
  apenas se houver justificativa de performance.
...
- Nomes de função genéricos
  ([`cvm_fetch()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_fetch.md))
  devem aceitar dataset de qualquer classe
- Schemas devem ser definidos em formato que escale para qualquer classe
...
- Genérica:
  `cvm_fetch(dataset, table, companies, years, source, on_error, validate, ...)`.
- Aliases tipados v0.1:
  [`cad_fetch()`](https://sidneybissoli.github.io/cvmdata/reference/cad_fetch.md),
  `itr_fetch()`, `dfp_fetch()`, `fre_fetch()`.
- Aliases tipados v0.4+: `fii_fetch()`, `fidc_fetch()`, `fie_fetch()` (e
  equivalentes a serem nomeados na expansão).
- Funções de descoberta:
  [`cvm_datasets()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_datasets.md),
  `cvm_tables(dataset)`, `cvm_dictionary(dataset, table)`.
- Cache:
  [`cvm_cache_path()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_cache_path.md),
  `cvm_cache_set_path(path)`, `cvm_cache_clear(what = "all", ...)`,
...
- Toda afirmação factual sobre a CVM, schema de dados, comportamento de
  endpoints, ou política regulatória deve vir com fonte verificável (URL
  específica) ou marcação explícita `[INFERIDO]` / `[NÃO VERIFICADO]`.
...
fetching), preferir verificar empiricamente em vez de citar documentação
secundária. Quando não houver, declarar a limitação.
...
Estas decisões NÃO devem ser questionadas em rodadas futuras a menos que
apareça evidência nova substancial. Vieram de seis sub-rodadas de
...
- API: Padrão C híbrido (genérica
  [`cvm_fetch()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_fetch.md) +
  aliases tipados; nomes travados na Rodada 2.5 — vide seção “Naming
  canônico” acima)
...
- CNPJ armazenado **com pontuação** (como vem da CVM); função utilitária
  [`cnpj_clean()`](https://sidneybissoli.github.io/cvmdata/reference/cnpj_clean.md)
  exportada para gerar versão sem pontuação.
...
- Schema do
  [`cvm_dictionary()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_dictionary.md):
  5 colunas (`campo`, `descricao`, `tipo_dados`, `dominio`, `tamanho`,
  `precisao`, `scale`), todas derivadas do dicionário oficial CVM.
...
- Decisões importantes registradas em arquivos markdown adicionados aos
  arquivos do projeto.
- Subsessões temáticas quando tópico merece foco isolado (ex: Rodada 2.5
  para naming).
...
R 4.x (mínimo 4.1 para `|>` nativo); tidyverse core; `arrow`; `duckdb`;
`pointblank`; `testthat` 3.x; `roxygen2`; `pkgdown`; `lintr`; `styler`;
`covr`; GitHub Actions; renv para desenvolvimento.

## Não fazer

- Não inventar respostas, fontes, URLs, schemas.
- Não simplificar excessivamente assuntos técnicos.
- Não assumir que o usuário precisa de explicação básica de conceitos R,
  tidyverse, CRAN policy, ou desenvolvimento de pacotes.
...
Documentos de referência produzidos em rodadas anteriores, presentes nos
arquivos do projeto. Quando relevante, consultá-los antes de propor
mudanças:

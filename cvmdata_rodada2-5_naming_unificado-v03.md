...
> (Rodada 2.6, 2026-05-18). **Idioma deste documento**: português
> brasileiro. **Status**: travado. Não reabrir sem evidência substancial
> nova. **Substitui em parte**: `cvmdata_rodada2_plano_arquitetural.md`
> (decisões de naming). Em caso de conflito entre os dois documentos,
> prevalece este. **Pré-requisitos**: `cvmdata_rodada1_fechamento.md`
> (escopo macro) e `cvmdata_rodada2_plano_arquitetural.md` (plano
> técnico).

> ### Refinamentos incrementais — Rodada 2.6 (2026-05-18)
>
> Os ajustes abaixo derivaram de verificação empírica direta dos
> arquivos do Portal de Dados Abertos CVM em 2026-05-18, na subsessão
> Rodada 2.6 (schemas). São **incrementais**: não reabrem nenhuma das 26
> decisões da Rodada 2.5, apenas adicionam precisão onde a redação
> original generalizava sem evidência.
>
> - §3 (Colunas-chave universais): adicionada §3.0 com caveat empírico
>   sobre divergência de convenção de naming entre
>   CAD/ITR/DFP/FRE-header e FRE-detail. Decisão: preservação estrita
>   (sem renomeação).
...
>   metadados de submissão chama-se `submissao` em ITR, DFP e FRE. Vide
>   §2 do fechamento da Rodada 3.0.
> - §4.1 (Tabelas ITR e DFP): adicionada linha `submissao` à tabela de
>   18 tabelas espelhadas; total ajustado para 19 tabelas espelhadas.
>   Vide §2 do fechamento da Rodada 3.0.
> - §4.2 (Tabelas FRE): “a confirmar nome empírico” substituído por
>   `submissao` na Categoria A — Cabeçalho. Total FRE: 36 tabelas reais
>   (1 `submissao` + 35 detalhes). Nota sobre tabelas-fantasma do META
>   atualizada de “14 a mais que CSVs no release 2024” para “22 a mais
>   que CSVs no release 2024” (achado refinado em 2026-05-19; vide §5.2
>   do fechamento da Rodada 3.0).
> - §7.4 (Dicionário CVM em snapshot separado): formato fechado em CSV
>   UTF-8 com delimitador `,`. Vide §3 do fechamento da Rodada 3.0.
> - §7.8 (nova subseção): snapshot de codelists empíricas em
>   `inst/extdata/cvm_codelists_snapshot.csv`, schema mínimo de 4
>   colunas obrigatórias mais campo opcional `first_seen_year` na v0.1;
>   geração por varredura histórica completa com cache por hash. Vide §4
>   do fechamento da Rodada 3.0.
> - §11 (Pendências empíricas): itens resolvidos nesta sessão marcados
>   como `[RESOLVIDO 3.0]` ou `[REFINADO 3.0]`; achados novos
>   adicionados como `[ADIÇÃO 3.0]` em §11.3 e §11.4.

------------------------------------------------------------------------

## Sumário

- [0. Régua geral](#id_0-r%C3%A9gua-geral)
- [1. Funções públicas](#id_1-fun%C3%A7%C3%B5es-p%C3%BAblicas)
- [2. Argumentos canônicos](#id_2-argumentos-can%C3%B4nicos)
- [3. Colunas-chave universais](#id_3-colunas-chave-universais)
- [4. Nomes de tabelas](#id_4-nomes-de-tabelas)
- [5. Atributos do tibble
  retornado](#id_5-atributos-do-tibble-retornado)
- [6. Mensagens, erros, verbosidade](#id_6-mensagens-erros-verbosidade)
- [7. YAMLs de schema interno](#id_7-yamls-de-schema-interno)
- [8. Funções utilitárias](#id_8-fun%C3%A7%C3%B5es-utilit%C3%A1rias)
- [9. Transformações que alteram a forma do
  tibble](#id_9-transforma%C3%A7%C3%B5es-que-alteram-a-forma-do-tibble)
- [10. Histórico das 26
  decisões](#id_10-hist%C3%B3rico-das-26-decis%C3%B5es)
- [11. Pendências empíricas para Rodada
  3](#id_11-pend%C3%AAncias-emp%C3%ADricas-para-rodada-3)
...
> **Se vem da CVM, fica como na CVM** (apenas com normalização
> tipográfica para snake_case minúsculo quando o original está em
...
A palavra `dataset` entrou no vocabulário técnico PT da comunidade R sem
tradução natural. Em consequência:

- O **argumento** `dataset` em
  ...
  permanece em inglês.
- A **coluna** equivalente no retorno de
  [`cvm_datasets()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_datasets.md)
  chama-se `conjunto_dados` — em PT, fiel à nomenclatura oficial CVM no
  portal `dados.cvm.gov.br`.
...
Aplicação: todas as funções públicas do pacote começam com `cvm_` ou com
o prefixo do dataset (`cad_`, `itr_`, `dfp_`, `fre_`, e futuros `fii_`,
`fidc_`, etc.).

### 0.5 Régua para argumentos de comportamento adverso

Argumentos que controlam comportamento em situações adversas usam **três
níveis paralelos** (mesma régua mental para o usuário):
...
Aceita dataset de qualquer classe (companhias na v0.1; fundos na v0.4+).
Argumentos `companies`/`years` se aplicam à classe companhias; classes
futuras podem ter outros argumentos análogos via dispatch interno
(decisão arquitetural mantida da Rodada 2).
...
|----|----|----|
| [`cad_fetch()`](https://sidneybissoli.github.io/cvmdata/reference/cad_fetch.md) | `cad` | Cadastro de companhias abertas |
...
indicando uso correto e função relacionada para listagem.
...
Argumento `source` em
[`cvm_fetch()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_fetch.md)
continua funcionando como override pontual da configuração global.
...
Argumentos que aparecem em mais de uma função e devem ser **idênticos em
nome, tipo, default e semântica** em todas as funções:
...
| `"warn"` | Warning via [`rlang::warn()`](https://rlang.r-lib.org/reference/abort.html)/[`cli::cli_warn()`](https://cli.r-lib.org/reference/cli_abort.html); pacote tenta fallback (portal → mirror); retorna o que conseguiu | Warnings por entidade que falhou; tibble retorna com as que deram certo |
...
|----|----|----|----|
| `cnpj_cia` | character | CNPJ_CIA | **Com pontuação** como vem da CVM (`"12.345.678/0001-90"`); usar [`cnpj_clean()`](https://sidneybissoli.github.io/cvmdata/reference/cnpj_clean.md) para versão sem pontuação |
...
> `id_doc`, `dt_receb`, `link_doc`) chama-se **`submissao`** em ITR, DFP
> e FRE. Verificação empírica em 2026-05-19 confirmou estrutura idêntica
> de 9 campos nos três datasets, na mesma ordem. Vide §2 do fechamento
> da Rodada 3.0 para justificativa completa.
...
1.  **FRE-header** (`fre_cia_aberta_<ano>.csv`, uma linha por
    submissão): usa SCREAMING_SNAKE_CASE igual a CAD/ITR/DFP —
    `CNPJ_CIA`, `DT_REFER`, `DENOM_CIA`, `CD_CVM`, `VERSAO`,
...
deve ser lida como “presentes em CAD + ITR + DFP + FRE-header”;
FRE-detail opera com o conjunto análogo da segunda coluna da tabela
acima.

### 3.1 Colunas adicionais em tabelas contábeis (ITR/DFP)

Em tabelas das demonstrações financeiras (BPA, BPP, DRE, DRA, DFC, DMPL,
DVA, todas em variantes `_con` e `_ind`):
...
foi decidido na Rodada 3.0 após verificação empírica direta do Portal de
Dados Abertos CVM (vide §2 do fechamento da Rodada 3.0). Histórico da
pendência preservado em §11.

### 4.2 Tabelas FRE (Formulário de Referência)

As 36 tabelas FRE reais (1 `submissao` + 35 detalhes) estão agrupadas em
categorias A-J no plano arquitetural §4.4.2 (referência interna; **não
exposta como coluna em
[`cvm_tables()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_tables.md)**).
Nomes em PT, snake_case minúsculo:

#### Categorias e exemplos de tabelas

- **Categoria A — Cabeçalho**: `submissao` — registro de submissão do
  documento à CVM (metadados administrativos); estrutura idêntica à de
  ITR e DFP; nome decidido na Rodada 3.0 após verificação empírica (vide
  §2 do fechamento da Rodada 3.0)
...
Rodada 3 (verificação empírica do conteúdo de cada arquivo CSV-CVM antes
de travar nome canônico). A `submissao` (Categoria A) já está travada
(vide §2 do fechamento da Rodada 3.0). A régua a aplicar nas demais é a
desta seção: PT, snake_case, preservar siglas e jargão.
...
[`cvm_fetch()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_fetch.md)
e aliases carrega **5 atributos em inglês** com metadados de
proveniência:
...
Implementação via
`rlang::abort(class = c("cvmdata_error_<tipo>", "cvmdata_error"), ...)`.
Captura específica:
...
- Função preferida para erros:
  [`cli::cli_abort()`](https://cli.r-lib.org/reference/cli_abort.html)
  ou [`rlang::abort()`](https://rlang.r-lib.org/reference/abort.html).
- Função preferida para informações:
  [`cli::cli_inform()`](https://cli.r-lib.org/reference/cli_abort.html)
  ou [`rlang::inform()`](https://rlang.r-lib.org/reference/abort.html).
- Função preferida para warnings:
  [`cli::cli_warn()`](https://cli.r-lib.org/reference/cli_abort.html) ou
  [`rlang::warn()`](https://rlang.r-lib.org/reference/abort.html).
- [`print()`](https://rdrr.io/r/base/print.html) e
  [`cat()`](https://rdrr.io/r/base/cat.html) proibidos fora de métodos
  `print.*`/`str.*`.
- Idioma: **inglês**.
...
[`cli::cli_inform()`](https://cli.r-lib.org/reference/cli_abort.html)
internamente, o que respeita automaticamente `rlib_message_verbosity`
(padrão tidyverse compartilhado) por baixo dos panos. Quando
`rlib_message_verbosity = "quiet"`, todas as mensagens informativas são
silenciadas independentemente do `cvmdata.verbosity`.

Nível `"debug"` é exclusivo do `cvmdata.verbosity` (expõe detalhes HTTP,
URLs, etc.). Não há equivalente em `rlib_message_verbosity`.
...
- **`cvm_file_pattern`** é o nome do arquivo CSV. Quando o dataset tem
  particionamento temporal, contém `{year}` (ou outro placeholder
  futuro) interpolado pelo reader.
...
data (formato ISO-8601). Tornar `convert_to_date` uma ação declarada por
coluna inflaria os YAMLs sem ganho — `bpa_con` teria 2 entries só para
isso, e `administrador_membro_conselho_fiscal` teria 5. Portanto:
...
Adições futuras de actions devem ser documentadas como incrementos desta
tabela.

### 7.4 Dicionário CVM em snapshot separado

Para evitar duplicação massiva, o dicionário oficial CVM **não** é
replicado nos YAMLs por tabela. Em vez disso, vive em snapshot único em
`inst/extdata/cvm_dictionary_snapshot.csv`, formato CSV UTF-8 com
delimitador `,` (vírgula), atualizado periodicamente por GitHub Action.
Formato fechado na Rodada 3.0 (vide §3.4 do fechamento da Rodada 3.0). A
função
[`cvm_dictionary()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_dictionary.md)
lê do snapshot.
...
a CVM apenas declara `Domínio: Alfanumérico` para a maioria dos domínios
enumerados. As codelists têm de ser **varridas empiricamente** dos CSVs,
conforme princípio fixado em §11.3 (`[RESOLVIDO 2.6]`). Esta subseção
fecha o formato e o schema desse artefato.

**Localização e formato**: `inst/extdata/cvm_codelists_snapshot.csv`,
encoding UTF-8, delimitador `,` (vírgula). Mesmo formato e mesmo lugar
do `cvm_dictionary_snapshot.csv` (vide §7.4) para coerência de
manutenção.
...
A chave composta é `(dataset, table, column)` — o escopo de uma codelist
é a coluna **dentro** de uma tabela, conforme princípio fixado em §11.3
(`[RESOLVIDO 2.6]`: “codelist tem escopo (tabela, coluna), não coluna
isolada”).

**Schema deferido para v0.2+**:

- `frequency` (integer, contagem de ocorrências): muda em toda execução
  do workflow ETL, invalidando a vantagem de diff Git legível do CSV.
  Caso de uso “identificar valores raros” pode ser atendido pelo próprio
  reader (`distinct() |> count()` sobre o tibble retornado por
  [`cvm_fetch()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_fetch.md)).
- `last_seen_year` (integer, último ano observado): mesmo problema de
  `frequency` — sobe a cada release para valores ainda presentes.
  Alternativa preferível em v0.2+: quando um valor deixa de aparecer em
  N releases consecutivos, removê-lo do snapshot (com registro em
  NEWS.md).

`first_seen_year` é incluído na v0.1 porque é o único campo de metadados
temporal que **não muda** nas execuções rotineiras do workflow (só muda
quando um valor inédito aparece pela primeira vez) — preserva a vantagem
de diff Git legível.

**Estratégia de geração**: varredura histórica completa do primeiro
release disponível por dataset até o release mais recente. Custo único
na geração inicial; custo incremental por release novo via cache por
hash do CSV (`data-raw/codelist_csv_hashes.json`). Detalhes operacionais
em §4.4 do fechamento da Rodada 3.0.

Referência cruzada: §4.4 do fechamento da Rodada 2.6 fixou o princípio
“codelists vêm de varredura empírica, não do META”; §4.5 do fechamento
da Rodada 2.6 fixou “codelist tem escopo (tabela, coluna), não coluna
isolada”. Esta subseção materializa ambos os princípios em forma de
artefato versionado.
...
conteúdo de células). Toda transformação aqui listada deve ter reflexo
no YAML mínimo da tabela afetada (seção `transformations:`) e em
`@details` do help da função alias (`itr_fetch`, `dfp_fetch`).
...
**Regra**: perguntar ao mantenedor caso a caso. Não há régua automática
de remoção. Default conservador é manter a coluna mesmo sendo constante.
...
para cada combinação de `cnpj_cia` + `dt_refer` + chaves específicas da
tabela. Comportamento default; usuário pode desativar via argumento a
definir na Rodada 3.
...
foram **revertidas**, para futura auditoria editorial. Cada item lista o
**estado final** e, quando relevante, o histórico de reaberturas.
...
| 4 | Nome da função de dicionário | [`cvm_dictionary()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_dictionary.md) |
| 6 | Comportamento de [`cvm_dictionary()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_dictionary.md) sem argumentos | Erro informativo via [`cli::cli_abort()`](https://cli.r-lib.org/reference/cli_abort.html) apontando para [`cvm_datasets()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_datasets.md) / [`cvm_tables()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_tables.md) |
| 12 | Família de funções de cache | 4 funções: [`cvm_cache_path()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_cache_path.md), [`cvm_cache_set_path()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_cache_set_path.md), [`cvm_cache_clear()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_cache_clear.md), [`cvm_cache_info()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_cache_info.md) |
| 13 | Família de funções de source | 2 funções: [`cvm_source_get()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_source_get.md), [`cvm_source_set()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_source_set.md); domínio fechado `"auto"`/`"portal"`/`"mirror"` |
| 18 | Valores de células categóricas | **Preservados em PT** como vêm da CVM (não traduzidos para EN) |
| 19 | Schema do [`cvm_dictionary()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_dictionary.md) | 7 colunas conforme dicionário oficial CVM: `campo`, `descricao`, `dominio`, `tipo_dados`, `tamanho`, `precisao`, `scale` |
...
| 9 | Coluna `entity_class` em [`cvm_datasets()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_datasets.md) | A CVM já fornece taxonomia oficial via `grupo` (~54 valores) | Coluna chama-se **`grupo`** e herda valores da CVM |
| 10 | [`cvm_datasets()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_datasets.md) com 6 colunas (`dataset`, `entity_class`, `description_en`, `description_pt`, `n_tables`, `first_year`) | Mudança de régua de idioma + adoção do `grupo` | 5 colunas em PT: `conjunto_dados`, `grupo`, `descricao`, `n_tabelas`, `primeiro_ano` |
| 11 | [`cvm_tables()`](https://sidneybissoli.github.io/cvmdata/reference/cvm_tables.md) com 6 colunas (D — variante com `entity_class`) | Mudança de régua de idioma + simplificação | 3 colunas em PT: `tabela`, `descricao`, `n_colunas` |
...
- **Régua de idioma diferenciada** (princípio reitor): cunhada no fluxo
  da sessão; documentada em §0.1.
- **CNPJ preservado com pontuação**: `cnpj_cia` armazenado tal como vem
  da CVM (`"12.345.678/0001-90"`).
- **[`cnpj_clean()`](https://sidneybissoli.github.io/cvmdata/reference/cnpj_clean.md)
  como função utilitária pública**: exportada na v0.1.
...
Atualizado pós-Rodada 2.6: itens fechados marcados com
`[RESOLVIDO 2.6]`, parciais com `[PARCIAL 2.6]`.

### 11.1 Nomeação

- **\[RESOLVIDO 3.0\]** Nome do arquivo de cabeçalho de ITR/DFP.
  Verificação empírica em `itr_cia_aberta_2024.zip`: o arquivo de
  cabeçalho é `itr_cia_aberta_<ano>.csv` (sem sufixo de demonstração
  contábil). Tem 9 campos: `CNPJ_CIA`, `DT_REFER`, `VERSAO`,
  `DENOM_CIA`, `CD_CVM`, `CATEG_DOC`, `ID_DOC`, `DT_RECEB`, `LINK_DOC`.
  **Nome canônico-PT decidido na Rodada 3.0: `submissao`**; aplica-se
  também a FRE-header (estrutura idêntica de 9 campos confirmada
  empiricamente em 2026-05-19). Justificativa completa em §2 do
  fechamento da Rodada 3.0. Histórico: este item passou pela Rodada 2.6
  como `[RESOLVIDO 2.6]` com caveat “nome canônico-PT permanece a
  decidir”; reaberto apenas para o nome canônico, agora travado.
- Nomes finais individuais das ~36 tabelas FRE. **\[PARCIAL 2.6\]**
  Inventário empírico no `fre_cia_aberta_2024.zip` confirma 36 CSVs com
  nomes `fre_cia_aberta_<table>_2024.csv`. A régua canônica “fica como
  na CVM” aplicada produz mapeamento direto (nome da tabela = sufixo do
  CSV). Nota: o META zip tem 22 dicionários a mais que CSVs no release
  2024 (achado refinado em 2026-05-19, vide §5.2 do fechamento da Rodada
  3.0; o valor anterior “14 a mais” subestimava a diferença). Indicam
  tabelas pré-anunciadas pela CVM mas ainda não populadas. Decisão
  Rodada 3: incluir apenas tabelas presentes em pelo menos um release.
...
- **\[RESOLVIDO 2.6\]** `cd_cvm` admite zeros à esquerda em ITR
  (zero-padded a 6 dígitos: `000094`..`001023`) **mas não em CAD** (sem
  padding: `35`..`28010`). Coerção para inteiro perderia o padding do
  ITR. Mantida decisão **character** em todos os datasets.
- **\[PARCIAL 2.6\]** `versao`. ITR/BPA 2024: valores empíricos 1..5;
  META declara `smallint`. Mantido `character` por princípio
  “identificador → character” e proteção contra mudanças futuras de
  formato. Sem evidência para reabrir.
- **\[ADIÇÃO 2.6\]** **Aviso geral sobre `Tipo Dados` do META CVM**: não
  ...
  - Regra de implementação Rodada 3: para campos identificadores (CNPJ,
    CD_CVM, CEP, telefones, CPF), forçar leitura como character **mesmo
    quando o META declara numeric**, independente de dataset.

### 11.3 Conteúdo

- Conteúdo exato do dicionário CVM (`meta_*.txt`) para as ~75 tabelas do
  v0.1. **\[PARCIAL 2.6\]** 3 dicionários inspecionados a fundo (CAD,
...
- **\[RESOLVIDO 2.6\]** **CVM não publica codelist enumerada no META**
  ...
  enumerados maiores (`ORDEM_EXERC`, `ESCALA_MOEDA`,
  `Orgao_Administracao`, `SIT`, etc.), o META apenas declara
  `Domínio: Alfanumérico`. Codelists têm que ser **varridas
  empiricamente do CSV**, não extraídas do META.
- **\[RESOLVIDO 2.6\]** **Codelist tem escopo (tabela, coluna), não
  coluna isolada**. Mesma coluna `Orgao_Administracao` aparece com
  codelists diferentes em tabelas diferentes do FRE. Exemplo verificado
  em 2024:
  - `administrador_membro_conselho_fiscal`: 4 valores
    (`"Conselho Fiscal"`,
  ...
  - `administrador_declaracao_genero`: 5 valores
    (`"Conselho Fiscal - Efetivos"`, `"Conselho Fiscal - Suplentes"`,
    `"Conselho de Administração - Efetivos"`,
    `"Conselho de Administração - Suplentes"`, `"Diretoria"`). Estrutura
    interna do snapshot de codelists em Rodada 3 deve refletir isso
    (chave composta `dataset` + `table` + `column`).
...
- **\[ADIÇÃO 3.0\]** **Oito tabelas FRE com dados publicados mas sem
  META correspondente**. Verificação empírica em 2026-05-19 detectou que
  o `fre_cia_aberta_2024.zip` publica 8 CSVs sem dicionário homólogo no
  `meta_fre_cia_aberta.zip`: `administrador_PCD`, `empregado_PCD`,
  ...
  `empregado_posicao_local`. Cobertura faltante do META é defeito
  conhecido da CVM (juntando-se aos defeitos documentados em §11.5).
  Política do reader para esse caso fica para Rodada 3.0.2 (subsessão
  temática dedicada); diretriz preliminar em §6 item 8 do fechamento da
  Rodada 3.0 (leitura sem validação contra META, com warning
  informativo; `cvm_dictionary_url: null` no YAML; `descricao = NA` para
  todas as colunas no snapshot de dicionário).
...
- **\[RESOLVIDO 3.0\]** Nome final do snapshot de dicionário:
  ...
  (benchmark empírico de 91 KiB / 3.4 ms; trade-off de tamanho vs diff
  Git legível decidido a favor de CSV). Histórico: este item foi listado
  como pendência na Rodada 2.6.
- **\[REFINADO 3.0\]** Workflow GitHub Action para atualização periódica
  do snapshot de dicionário. Design fechado: trigger por hash dos
  `meta_*.txt` baixados do Portal CVM (armazenados em
  `data-raw/dictionary_hashes.json`); quando algum hash muda, regenera o
  snapshot e abre PR para revisão humana antes do merge. Integrado ao
  workflow ETL semanal já travado na Rodada 1 (terças 07:00 UTC).
  Implementação fica no escopo da Rodada 3 plena.
- **\[ADIÇÃO 3.0\]** Snapshot de codelists empíricas:
  ...
  Geração por varredura histórica completa com cache por hash do CSV
  (`data-raw/codelist_csv_hashes.json`). Detalhamento em §4 do
  fechamento da Rodada 3.0 e em §7.8 deste documento. Implementação fica
  no escopo da Rodada 3 plena.
- **\[PARCIAL 2.6\]** Padrão dos arquivos `cvm_file_pattern` em cada
  YAML de schema. Confirmados para 3 tabelas (vide YAMLs entregues na
...
- **\[PARCIAL 2.6\]** URL exata do dicionário CVM (`cvm_dictionary_url`)
  ...
  - CAD:
    ...
    (TXT direto)
  - ITR:
    `https://dados.cvm.gov.br/dados/CIA_ABERTA/DOC/ITR/META/meta_itr_cia_aberta_txt.zip#meta_itr_cia_aberta_<sigla>.txt`
    (ZIP com um TXT por demonstrativo, não por variante con/ind)
  - FRE:
    `https://dados.cvm.gov.br/dados/CIA_ABERTA/DOC/FRE/META/meta_fre_cia_aberta.zip#meta_fre_cia_aberta_<table>.txt`
    (ZIP com um TXT por tabela)
  - DFP: padrão análogo ao ITR a confirmar na Rodada 3.

### 11.5 Validação de design

- Verificar empiricamente se as transformações documentadas em §9 cobrem
  todas as transformações que o pacote efetivamente aplica (auditoria
  cruzada).
...
- **\[ADIÇÃO 2.6\]** **Defeitos de qualidade conhecidos da CVM**,
  descobertos durante a verificação. Documentar em vinheta:
  ...
  - CEPs perdem zeros à esquerda no CAD (`Tipo Dados: numeric` na
    fonte). Mantido como vem da CVM em v0.1; possível utilitária
    `cep_pad()` em v0.2+.
  - Empty `Descrição`/`Domínio` em alguns campos do META FRE (ex.:
    `Data_Inicio_Primeiro_Mandato` em
    `administrador_membro_conselho_fiscal`). Reader deve aceitar campos
    com metadados parciais.
- **\[ADIÇÃO 2.6\]** **Precisão numérica de `VL_CONTA`**: o META declara
  ...
  precisão para valores extremos. Validar em Rodada 3 se algum valor
  real na base ultrapassa o limite; se sim, decidir entre uso de
...
2.  Geração programática dos ~75 YAMLs de schema a partir do snapshot de
    dicionário CVM.

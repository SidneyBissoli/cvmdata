...
`cvm_fetch()`, com filtros consistentes para companhias (CNPJ / CD_CVM /
texto livre com expansão de abreviações), cache HTTP com revalidação por
...
(<https://dados.cvm.gov.br/>). Um mirror gerenciado pelo pacote
publicado em GitHub Releases serve como fallback automático quando um
arquivo upstream fica indisponível. Todo tibble retornado carrega
atributos de proveniência (`source`, `fetched_at`, `dataset`, `table`,
`package_version`).
...
Pull requests são bem-vindas. Veja [CONTRIBUTING.md](CONTRIBUTING.md)
...
Contribuidor](CODE_OF_CONDUCT.md) (em inglês). Ao contribuir, você
concorda em respeitar seus termos.
